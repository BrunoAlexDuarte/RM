...
%
% Modifications by v. santos (vitor@ua.pt, March 2023, 2024): Versions: 3.0
%
% ===============================================================================================
% 
...
subplot(1,2,1)
set(gcf, 'Position',  [257 214 1291 653])
...
title("Before update - Iteration " + step_no)
 %   pause
    ...
    for j = 1:size(particle,2)
        ...
        dx = 0.5*cos(particle(j).pos(3));
        dy = 0.5*sin(particle(j).pos(3));
        quiver(particle(j).pos(1),particle(j).pos(2), dx, dy, 0, 'Color', 'g')
    ...
    dx = 0.5*cos(robot_estimate.pos(3));
        ...
        quiver(robot_estimate.pos(1),robot_estimate.pos(2),dx, dy, 0, 'Color', 'b','linewidth',2)
...
title("After update - Iteration " + step_no)
